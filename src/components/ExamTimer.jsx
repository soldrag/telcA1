...
export default function ExamTimer({
  ...
  useEffect(() => {
    ...
    if (isTimed) {
      ...
      // Untimed practice mode: counts elapsed time
      const timer = setInterval(() => {
  ...
  }, [isTimed, isPaused, isSubmitted, secondsLeft, onTimeUp, setSecondsLeft, setSecondsElapsed]);
  ...
  const isCritical = secondsLeft <= 5 * 60; // less than 5 minutes
    ...
    <Card className="p-4 shadow-xs">
      ...
      {isPaused && !isSubmitted && (
        <div className="mt-3 p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-center justify-between text-xs text-amber-800 font-medium">
          <span>Экзамен на паузе. Нажмите «Продолжить», чтобы вернуть отсчёт времени.</span>
        </div>
      )}
    </Card>
  );
